...
public abstract class AbstractDAO extends SQLiteOpenHelper {
    ...
    public AbstractDAO(Context context) {
        super(context, DB_FILE_NAME, null, 10);// 7
    }
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String createLoanView = "CREATE VIEW LOAN_VIEW AS SELECT loan._id AS _id, item.NA_TITLE AS NA_NAME, loan.FG_STATUS AS FG_STATUS, loan.DT_LENT AS DT_LENT, friend.CONTACT_ID AS CONTACT_ID, DT_RETURN AS DT_RETURN FROM LOAN_HISTORY loan, ITEM item, FRIEND friend WHERE loan.id_friend = friend._ID AND loan.id_item = item._ID;";
